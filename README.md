...
2. 首页“记下此刻”可以先写感受，再决定收进人生之书或记忆之家；
3. 个人片段只进入当前用户的书，并可定向分享给一位家人阅读；
4. 用户在记忆之家回答一个关于家人的问题，用“我们”讲述；
5. 家庭片段经确认后进入共同时间线，可按家庭成员筛选；
6. 两套片段、草稿和生成来源在领域层隔离，不能互相串入。

未经确认的家庭投稿在确认前只对投稿人和确认者可见。个人故事始终归作者本人；定向分享只授予指定家人阅读权，不会把故事送入对方书稿或家庭时间线。家庭共同记忆不会被用于生成任何人的人生之书。
...
内容边界在当前单设备演示中由领域层统一执行：`personal` 片段只进入作者自己的书和草稿，定向分享只允许零或一位接收者；`family` 片段只在确认且家庭可见后进入记忆之家。异常缓存和多个接收者字段会失败关闭，不能扩大个人故事的阅读范围。旧缓存中没有作用域的片段按家庭记忆处理，避免误入个人书。正式多人版本仍需通过微信身份、云端访问控制和删除撤回来执行同一规则。
...
随手记路径：`人生之书 → 记下此刻 → 写下感受 → 选择故事归属 → 仅自己／指定家人 → 保存`

个人采访路径：`人生之书 → 写我的故事 → 个人采访 → 保存到我的书 → 整理章节 → 查看个人来源`

定向阅读路径：`切换到接收者身份 → 家人分享给我的 → 阅读原文（不进入我的书）`
...
| 回忆采访 · 每轮只追问一个方向 | 随手记 · 先记录再决定归属 |
| --- | --- |
| ![回忆采访](docs/images/05-interview.png) | ![记下此刻](docs/images/06-quick-note.png) |
