...
> **2026-09-15 更新：照片改存云端（问题九）**
>
> 用户定了所有照片（包括书稿里的照片）存进腾讯云云存储，详见问题九方案 `docs/2026-09-15-import-and-cloud-photos-plan.md`（分支 `feat/import-and-cloud-photos`）。本节因此有以下变化，**以这里为准**：
>
> - **7.1 的传图方式作废**：「手机上压到 100KB」「照片以 base64 随云函数调用传」「只在内存里转交、不写数据库、不写云存储」「不走临时 CDN」。改为：`storyImages` 按 `{ familyId, photoIds }` 读问题九的 `photos` 记录，取云存储临时链接，交给 TokenHub 出图请求的 `images` 字段（最多 3 张，单张 ≤10MB，已删除的照片跳过）。参考图用显示图 `displayFileID`（长边 ≤1600），小图 `smallFileID` 留给看图写一句话和缩略图。图片任务只记 `photoIds` 和张数，不存照片内容。
> - **授权文案**：不再写「拾光家忆自己不保存照片」。照片存到哪里、存多久，由问题九和问题七定文案。**「把照片存到云端」和「把照片发给腾讯云 TokenHub 做 AI 处理」是两件事**：后者按个保法第 23 条仍须单独同意，看图写一句话和参考图出图都要过这一关。
> - **7.2 的看图模型**：改为 TokenHub 的 `hy-vision-2.0-instruct`（旧 `hunyuan-vision` 入口 2026-06-22 已下线，见第十六节）；图片以云存储临时链接传入，不再用 base64。`photo_insights.photoId` 指问题九 `photos` 记录里的照片。
> - **看图写一句话**由问题九牵头：在 `storyImages` 里加 `caption` 动作。问题五先把 `quality.js` 的看图调用抽成共用的 `vision.js`，问题九在其上实现，次数单独计，不占出图名额。
> - **7.3 长相护栏全部不变**：「识别长相」默认关闭；不自动判断照片里有没有人、是谁；参考图只用用户选的场景和物件照片。
