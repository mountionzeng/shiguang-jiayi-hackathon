...
function chinaDayKey(nowMs) { return new Date(nowMs + 8 * 60 * 60 * 1000).toISOString().slice(0, 10); }
function validateAccount(account, identity) {
  return Boolean(account && account.status === "active" && account.wxOpenId === identity.openid &&
    account.accountId === identity.accountId && account.primaryFamilyId === identity.familyId);
}
function assertServerReady() {
  if (process.env.AI_SERVER_RELEASE_READY !== "true") throw aiError("AI_RELEASE_NOT_READY", "在线 AI 尚未完成发布验收");
}
function assertExpectedApp(context) {
  ...
  if (!APP_ID.test(expectedAppId) || appId !== expectedAppId) throw aiError("APP_ID_MISMATCH", "当前小程序没有获得在线 AI 权限");
  return { appId, openid };
}
async function resolveActiveIdentity(db, context) {
  ...
  let account;
  try { account = (await db.collection("user_accounts").doc(accountDocumentId).get()).data; }
  catch { throw aiError("IDENTITY_UNLINKED", "账号还没有关联到企业小程序"); }
...
async function assertIdentityStillActive(db, identity) {
  let account;
  try { account = (await db.collection("user_accounts").doc(identity.accountDocumentId).get()).data; }
  catch { throw aiError("IDENTITY_REVOKED", "账号权限已经变化，请重新登录"); }
...
async function reserveAiRequest(db, identity, kind, nowMs = Date.now()) {
  ...
  await db.runTransaction(async transaction => {
    ...
    let account;
    try { account = (await ref.get()).data; }
    catch { throw aiError("IDENTITY_REVOKED", "账号权限已经变化，请重新登录"); }
    if (!validateAccount(account, identity)) throw aiError("IDENTITY_REVOKED", "账号权限已经变化，请重新登录");
    ...
    await ref.update({ data: { aiUsage: { dayKey, count: count + 1, lastAtMs: nowMs,
